...
export function generateQuotationHTML(
  ...
  const itemRows = quotation.items
    .map(
      (item) => `
    <tr>
      <td style="padding:12px 16px">${esc(item.name)}${item.hsnCode ? `<br/><span style="font-size:10px;color:#90A4AE">HSN: ${esc(item.hsnCode)}</span>` : ""}</td>
      <td class="center" style="padding:12px 16px">${item.quantity} ${esc(item.unit)}</td>
...
<title>Quotation ${esc(quotation.quoteNumber)}</title>
<style>
...
<div class="header">
  <div>
    <div class="brand-name">${esc(vendor.businessName)}</div>
    <div class="brand-details">${esc(vendor.vendorName)}<br/>${esc(vendor.whatsappNumber)}<br/>${esc(vendor.address)}${vendor.gstNumber ? `<br/>GST/TIN: ${esc(vendor.gstNumber)}` : ""}</div>
  </div>
  <div class="quote-info">
    <div class="quote-number">${esc(quotation.quoteNumber)}</div>
    <div class="quote-date">${date}</div>
  </div>
</div>
<div class="to-section">
  <div class="to-label">Quotation For</div>
  <div class="to-name">${esc(lead.name)}</div>
  ${lead.whatsappNumber ? `<div style="font-size:13px;color:#78909C;">${esc(lead.whatsappNumber)}</div>` : ""}
  ${lead.gstNumber ? `<div style="font-size:13px;color:#78909C;">GSTIN: ${esc(lead.gstNumber)}</div>` : ""}
</div>
<table>
  <thead>
    <tr>
...
${quotation.notes ? `<div class="notes"><strong>Notes:</strong> ${esc(quotation.notes)}</div>` : ""}
<div class="footer">Generated by BillBanao.ai</div>
</body>
</html>`;
}
  ...
  const itemRows = invoice.items
    .map(
      (item) => {
        ...
        return `
    <tr>
      <td style="padding:10px 12px">${esc(item.name)}</td>
      <td class="center" style="padding:10px 12px">${esc(item.hsnCode) || "-"}</td>
      <td class="center" style="padding:10px 12px">${item.quantity} ${esc(item.unit)}</td>
...
<div class="page">
  ...
  <div class="header-grid">
    <div class="box">
      ...
      <div class="business-name">${esc(vendor.businessName)}</div>
      <div style="margin-top:4px;color:#555;line-height:1.6">${esc(vendor.address)}</div>
      <div style="color:#555">${esc(vendor.vendorName)} · ${esc(vendor.whatsappNumber)}</div>
      ${vendor.gstNumber ? `<div class="gstin-badge">GSTIN: ${esc(vendor.gstNumber)}</div>` : ""}
    </div>
    <div class="box">
      <div class="box-title">Invoice Details</div>
      <div class="meta-grid">
        <span class="meta-label">Invoice No.</span><span><strong>${esc(invoice.invoiceNumber)}</strong></span>
        ...
        <span class="meta-label">Place of Supply</span><span>${esc(invoice.placeOfSupply)}</span>
      </div>
    </div>
  </div>
  <div class="header-grid">
    <div class="box">
      ...
      <div style="font-size:15px;font-weight:700">${esc(lead.name)}</div>
      <div style="color:#555;margin-top:2px">${esc(lead.whatsappNumber)}</div>
      ${lead.email ? `<div style="color:#555">${esc(lead.email)}</div>` : ""}
      ${invoice.buyerGstin ? `<div class="gstin-badge" style="margin-top:4px">GSTIN: ${esc(invoice.buyerGstin)}</div>` : (lead.gstNumber ? `<div class="gstin-badge" style="margin-top:4px">GSTIN: ${esc(lead.gstNumber)}</div>` : "")}
    </div>
    <div></div>
  </div>
  ...
  <div class="footer">
    This is a computer-generated invoice. · Generated by BillBanao.ai
  </div>
</div>
</body>
</html>`;
